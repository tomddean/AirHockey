...
import com.abstractorange.airhockey.android.util.Geometry.Circle;
...
public class ObjectBuilder {
	...
	private void appendCircle(Circle circle, int numPoints) {
		...
		for (int i=0; i <= numPoints; i++) {
			...
			vertexData[offset++] = circle.center.y;
			vertexData[offset++] = circle.center.x + circle.radius * FloatMath.sin(angleInRadians);
		}
	...
	private void appendOpenCylinder(Cylinder cylinder, int numPoints) {
		...
		for (int i = 0; i<=numPoints; i++) {
			...
			float zPosition = cylinder.center.x + cylinder.radius * FloatMath.sin(angleInRadians);
